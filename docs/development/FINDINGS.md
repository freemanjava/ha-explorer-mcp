...
**Outcome:** **Closed 2026-09-05 by `P5-01`** —
`docs/research/2026-09-05-zigbee-mesh-metric-normalization.md`, decided as
**D-05-5**. The answer is *flat analyzer plus a hint table*, not a
per-integration plugin seam: both integrations expose link quality and signal
strength as ordinary state-machine entities with recorder history, and disagree
only on the entity name, on whether a `device_class` marker exists, and on
whether the entity is enabled by default — differences a constant and
`MissingEvidence` absorb. The ZHA half is pinned-source evidence, not live: the
owner's installation has zero `zha` devices. One consequence outlives this
finding as **F-27**: `via_device_id` is a coordinator star on both
integrations, so D-05-3's shared-parent cluster annotation is vacuous for
Zigbee.

Previously: **Queued 2026-09-05 as `P5-01`**, at the head of Phase 05's queue. The
...
**What:** `P5-01` established
(`docs/research/2026-09-05-zigbee-mesh-metric-normalization.md`) that both
Zigbee integrations populate the device registry's `via_device_id` as a flat
star to the coordinator/bridge, not as a mesh routing parent. ZHA does it
explicitly — `homeassistant/components/zha/entity.py` `device_info` (core
`2026.8.3`, ~L140–148) sets `via_device_id` to the coordinator's device id for
every device whose ieee differs from the coordinator's. The owner's
Zigbee2MQTT installation shows the same N−1-of-N signature (27 of 28
populated).

D-05-3 decided that an outage cluster is annotated with what its members share
— "device, `via_device` parent, config entry, area" — and `P5-04`'s and
`P5-08`'s DoDs both treat a shared `via_device` parent as meaningful evidence
(`P5-08`: "a fixture where two devices share a parent produces a
topology-annotated cluster, and one where they do not produces the same time
cluster *without* the topology claim").

For Zigbee, every device shares that parent. The annotation is not merely weak,
it is vacuous: it fires on every Zigbee cluster and distinguishes nothing.

**Impact:** A cluster of Zigbee entities would carry a "these share a
`via_device` parent" annotation that is true of the whole network. That is
exactly the correlation-presented-as-cause this phase's first design note and
ADR-010 exist to prevent, and it would be *generated by our own code*, not by
an LLM misreading it — the failure mode D-05-1 hardens the types against,
arriving through the annotation instead. `P5-08`'s DoD as written is
...
candidate set is 1, and (2) whether the real Zigbee neighbour table (ZHA's
`neighbors` in `zha/devices`; Zigbee2MQTT's bridge topology) is reachable
inside the gateway allow-list, or whether mesh-parent evidence is simply
`MissingEvidence` in v1. One live measurement narrows (1) decisively and is
already wired: `cmd/spike`'s `probeMesh` now reports the number of *distinct*
